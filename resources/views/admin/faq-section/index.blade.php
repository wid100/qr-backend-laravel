...
@section('content')
    <div class="page-content">
        ...
        <nav class="page-breadcrumb d-flex justify-content-between">
            ...
            <a href="{{ route('admin.faq-section.create') }}" class="btn btn-primary">New Section</a>
        </nav>
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="dataTableExample" class="table">
                                ...
                                <tbody>
                                    @isset($faqSections)
                                    @foreach ($faqSections as $key => $item)
                                        <tr>
                                            <td>{{ $key+1 }}</td>
                                            <td>{{ $item->title }}</td>
                                            ...
                                            <td>
                                                <a href="{{ route('admin.faq-section.edit', $item->id) }}" class="btn btn-primary btn-icon  btn-sm"><i data-feather="edit"></i></a>

                                                @if (Auth::user()->role_id == 1)
                                                    <form id="delete_form_{{ $item->id }}"
                                                        action="{{ route('admin.faq-section.destroy', $item->id) }}"
